...
path <- "./cut_adapt_reads" # CHANGE to the directory containing your cutadapted fastq files when using your own data
filtpath <- file.path("./filtered") # Filtered files go into the filtered/ subdirectory
...
sample.names <- unlist(map(strsplit(basename(fns), "_"), 2)) # this pulls out just the basename
...
p1 / p2
ggsave("./graphs/unfiltered_quality_plots.png",dpi=500,height = 6,width = 6)

# FILTER AND TRIM ####

# here, we decide what filenames we want our filtered reads to be called
# in this case, we use the base name of the sample and save the filtered files into the "filtered" subdirectory of "./data/
...
saveRDS(out, "./cache/trackreads.RDS")
...
p3 / p4
ggsave("./graphs/filtered_quality_comparison.png",dpi=500,height = 6,width = 6)
...
write.csv(track, file = "./cache/16S_read_counts_at_each_step.csv", row.names = TRUE)

seqtab.nochim <- readRDS("./cache/seqtab.nochim.RDS")
# IMPORT METADATA ####
meta <- read_csv("./data/Strigosella_sam_data.csv")
ogmeta <- read_csv("../../metaamplicon-recipe/input/chagos_metadata_clean.csv")
...
df2 <- left_join(meta,df,by="SampleID")
...
saveRDS(seqtab.nochim,"./cache/seqtab.nochim.clean.RDS")

# Find and remove contaminants ####

# need to explain how to identify these in data set
seqtab.nochim <- readRDS("./cache/seqtab.nochim.clean.RDS")
...
write.csv(contams, file = "./cache/likely_contaminants.csv", row.names = TRUE)
...
# You can point this to any fasta-formatted training set (16S or ITS)
# Here, we are using the rdp traiing set of 16S sequences
taxa <- assignTaxonomy(seqtab.nochim, "./taxonomy/rdp_train_set_16.fa.gz", multithread=4) # on Windows, set multithread = FALSE

# Save intermediate taxonomy file
saveRDS(taxa, file = "./cache/RDP_Taxonomy_from_dada2.RDS")

# add_species names (this uses exact matching to add species assignments where possible)
taxa <- addSpecies(taxa, "./taxonomy/rdp_species_assignment_16.fa.gz")

# Save completed taxonomy file
saveRDS(taxa, file = "./cache/RDP_Taxonomy_from_dada2_sp.RDS")
...
saveRDS(ps,"./cache/ps_not-cleaned.RDS")
